...
parser.add_argument('--exp', type=str,
                    default='../models', help='experiment_name')
parser.add_argument('--model', type=str,
                    default='SSMIS-DC_10%BraTS2019', help='model_name')
def Inference(FLAGS):
    snapshot_path = "model/{}/{}".format(FLAGS.exp, FLAGS.model)
    num_classes = 2
    test_save_path = "output/{}/Prediction".format(FLAGS.exp)
    if os.path.exists(test_save_path):
        shutil.rmtree(test_save_path)
    os.makedirs(test_save_path)
    net = models_3D(in_channels=1, n_classes=2).to(device)
